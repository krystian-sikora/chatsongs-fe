...
const { isPlaying, playbackData, backtrack } = storeToRefs(pbStore)

const hasPrevious = computed(() => backtrack.value.length > 0)
const hasNext = computed(() => playbackData.value.song_queue.length > 1)
const isEmpty = computed(() => playbackData.value.song_queue.length === 0)
...
<template>
  <div class="*:w-5 *:h-6 -my-1 py-1 px-5 flex rounded-2xl bg-gray-100 shadow-accent space-x-2 w-fit">
